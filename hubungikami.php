...
if (isset($_POST['profil'])) { // mengecek apakah form variabelnya ada isinya

   $nama = $_POST['nama']; // isi varibel dengan mengambil data username pada form
   $subscribe = $_POST['subscribe']; // isi variabel dengan mengambil data password pada form
   $kritik = $_POST['kritik'];
   $email = $_POST['email']; // isi varibel dengan mengambil data username pada form
   

   $data = $conn->prepare('INSERT INTO profil (email, nama, subscribe, kritik) VALUES (?, ?, ?, ?)');

   $data->bindParam(1, $email);
   $data->bindParam(2, $nama);
   $data->bindParam(3, $subscribe);
   $data->bindParam(4, $kritik);
  
   $data->execute();
   echo "<script>alert('Terima kasih atas sarannya !');</script>";
}
  ...
  <body>
    <header>
      ...
      <div class="jumbotron warna-bg">
        <h1 class="display-4"><span class="font-weight-bold">Hubungi Kami</span></h1>
    </div>
    ...
    <div class="container">
      <div class="row">
        <div class="col-md-8 mb-4"><!-- Awal Col 8 -->
          <div class="card">
            <div class="card-body">
                <h2>Profil Wonderfull Papua</h2><br>
                <form method="post">
                  ...
                  <div class="form-group form-check">
                    ...
                    <label class="form-check-label" for="exampleCheck1">Subscribe Channel Wonderfull Papua</label>
                  </div>
                  <div class="form-group">
                    <label for="pesan">Kritik/Saran</label>
                    <textarea class="form-control" rows="5" name="kritik" id="kritik"></textarea>
                  </div>
                  <button type="submit" class="btn btn-primary" name="profil" id="profil">Submit</button>
                </form>
            </div>
          </div> 
        </div><!-- Akhir Col 8 -->
